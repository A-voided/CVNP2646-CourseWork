...
def main():
	...
	parser.add_argument('--output', '-o', default='outputs/priority_list.json', help='Output JSON file (default: outputs/priority_list.json)')
	...
	# Always write outputs to the outputs directory, regardless of the provided output path
	output_json_path = os.path.join(outputs_dir, os.path.basename(args.output))
	manager_output_path = os.path.join(outputs_dir, 'manager_output.txt')
	...
	try:
		...
		logging.info(f"Wrote prioritized vulnerabilities to {output_json_path}")
	...
	try:
		...
		logging.info(f"Wrote management summary to {manager_output_path}")
	except Exception as e:
